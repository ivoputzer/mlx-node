## Download
To download the assets required to run the tests, run `bash download.sh`

## Gemma
- https://huggingface.co/mlx-community/gemma-4-e4b-6bit
- https://huggingface.co/mlx-community/gemma-3-4b-it-4bit
- https://huggingface.co/mlx-community/gemma-2-2b-it-4bit

## Ministral
- https://huggingface.co/mistralai/Ministral-8B-Instruct-2410
- https://huggingface.co/mlx-community/Mistral-7B-Instruct-v0.3-4bit

## Qwen
- https://huggingface.co/Qwen/Qwen3.6-27B
- https://huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct
- https://huggingface.co/mlx-community/Qwen3.5-9B-6bit
- https://huggingface.co/mlx-community/Qwen2.5-Coder-1.5B-Instruct-4bit

## Meta
- https://huggingface.co/mlx-community/Meta-Llama-3.1-8B-Instruct-8bit
- https://huggingface.co/mlx-community/TinyLlama-1.1B-Chat-v1.0-4bit

## Deepseek
- https://huggingface.co/deepseek-ai/DeepSeek-V3
- https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash
- https://huggingface.co/mlx-community/DeepSeek-R1-Distill-Llama-8B-4bit
- https://huggingface.co/mlx-community/DeepSeek-R1-Distill-Qwen-7B-4bit

## OpenAI
- https://huggingface.co/openai/gpt-oss-20b

## IBM
- https://huggingface.co/mlx-community/Granite-4.1-8b-8bit

## Other
- https://huggingface.co/HuggingFaceTB/SmolLM2-135M-Instruct
- https://huggingface.co/microsoft/Phi-4-mini-instruct

# Jackrong
- https://huggingface.co/Jackrong/MLX-Qwen3.5-9B-DeepSeek-V4-Flash-6bit
- https://huggingface.co/Jackrong/MLX-Qwen3.5-9B-DeepSeek-V4-Flash-4bit
- https://huggingface.co/Jackrong/Qwopus3.5-9B-v3
- https://huggingface.co/Jackrong/MLX-Qwen3.5-9B-Claude-4.6-Opus-Reasoning-Distilled-8bit
